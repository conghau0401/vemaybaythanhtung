...
function catchuoi($chuoi,$gioihan){
	...
	*/
	...
	class func_index{
		...
		function upload_image($file, $extension, $folder, $newname=''){
			if(isset($_FILES[$file]) && !$_FILES[$file]['error']){
				...
				if($newname=='' && file_exists($folder.$_FILES[$file]['name']))
					...
					$_FILES[$file]['name'] = $newname.'_'.$this->bodautv($name).'.'.$ext;
				}
		...
		function getActive($alias = '',$lang = 'vn') {
			if($alias!='') {
				...
				$query1=$this->o_fet("select * from #_tintuc where alias_$lang = '$alias' ");
				$query2=$this->o_fet("select * from #_sanpham where alias_$lang = '$alias' ");

				// if(count($query1)>0 || count($query2)>0) {
				// 	if(count($query1)>0)  $id_loai=$query1[0]['id_loai']; else if(count($query2)>0) $id_loai=$query2[0]['id_loai'];	
		...
		function checkLink($text,$field,$id) {
			...
			$count=$this->num_rows("select * from #_category where $field='{$text}' $and");
			$count1=$this->num_rows("select * from #_sanpham where $field='{$text}' $and");
			$count2=$this->num_rows("select * from #_tintuc where $field='{$text}' $and");
				...
				foreach ($query as $key => $value) {
					...
					if(!empty($result)){
						foreach ($result as $key => $value) {
							array_push($array_temp, $value['page']);
						}
					}
				}								
			}
