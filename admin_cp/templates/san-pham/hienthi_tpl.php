...
<table class="table table-bordered table-hover">
	<thead>
		<tr>
			...
			<th style="width:6%">Tiêu biểu</th>
			<th style="width:6%">Mới</th>
	...
	<tbody>
		...
		<tr>
			...
			<!-- <td><?=$items[$i]['ma_sp']?></td> -->
			<td style="text-align:left" class="list-tour">
				<?php 
					
					$arr_list = explode(',',$items[$i]['id_loai']);
					$query = $d->o_fet("select * from #_category where FIND_IN_SET(id,'".$items[$i]['id_loai']."') order by id desc");
					$name = '';
					foreach($query as $value) {
						$name .= "<p>".$value['ten_vn']."</p>";
					}
					echo $name;
			...
			<td>
				<a href="index.php?p=san-pham&a=edit&id=<?=$items[$i]['id']?>&page=<?=@$_GET['page']?>">
					<?=($items[$i]['hinh_anh'] <> '')?"<img src='".URLPATH."thumb.php?src=".URLPATH."img_data/images/".$items[$i]['hinh_anh']."&w=60&h=40&zc=2'>":"<img src='".URLPATH."thumb.php?src=".URLPATH."images/no-image.png&w=40&h=40&zc=2'>";?>
				</a>
			</td>
