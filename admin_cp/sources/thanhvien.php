...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
		...
		if($id!=null){
			...
			if($d->update($data)){}
			$d->redirect("index.php?p=thanhvien&a=man&page=".@$_REQUEST['page']."");
		}
		...
		if(isset($_GET['seach'])){
			...
			if($seach == 'id'){
				...
				$items = $d->o_fet("select * from #_thanhvien where ten_vn like '%".$key."%'");
			}
		}
...
function xoadulieu(){
	...
	if(isset($_GET['id'])){
		...
		$hinhanh = $d->o_fet("select * from #_thanhvien where id = '".$id."'");
		...
		if($d->delete()){
			...
			$d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=thanhvien&a=man");
		}
	}else $d->transfer("Không nhận được dữ liệu", "index.php?p=thanhvien&a=man");
}
...
function xoadulieu_mang(){
	...
	if(isset($_POST['chk_child'])){
		...
		else $d->transfer("Xóa dữ liệu bị lỗi", "index.php?p=thanhvien&a=man");
	}else $d->redirect("index.php?p=thanhvien&a=man&page=".@$_REQUEST['page']."");
}
?>