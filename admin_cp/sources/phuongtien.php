...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
		...
		$items = $d->o_fet("select * from #_phuongtien order by id asc");
		...
		if(isset($_REQUEST['id'])){
			...
			$items = $d->o_fet("select * from #_phuongtien where id =  '".$id."'");
		}
	}
}
...
function luudulieu(){
	...
	if($id != '')
	{
		if(@$file = $d->upload_image("file", '', '../img_data/images/',$file_name)){
			...
			$hinhanh = $d->o_fet("select * from #_phuongtien where id = '".$id."'");
		...
		$d->setTable('#_phuongtien');
		$d->setWhere('id',$id);
		if($d->update($data)){
			$d->redirect("index.php?p=phuongtien&a=man&page=".@$_REQUEST['page']);
		}
		else{
			$d->alert("Cập nhật dữ liệu bị lỗi!");
			$d->redirect("Cập nhật dữ liệu bị lỗi", "index.php?p=phuongtien&a=man");
		}
	}
	else
	{
		...
		if($d->insert($data))
		{
			$d->redirect("index.php?p=phuongtien&a=man");
		}
		else{

			$d->alert("Thêm dữ liệu bị lỗi!");
			$d->redirect("Thêm dữ liệu bị lỗi", "index.php?p=phuongtien&a=man");
		}
	}
}
...
function xoadulieu(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete()){
			...
			$d->redirect("Xóa dữ liệu bị lỗi", "index.php?p=phuongtien&a=man");
		}
	}else {
...
function xoadulieu_mang(){
	...
	if(isset($_POST['chk_child'])){
		...
		$hinhanh = $d->o_fet("select * from #_phuongtien where id in ($chuoi)");
		if($d->o_que("delete from #_phuongtien where id in ($chuoi)")){
			//xoa hình ảnh
			foreach ($hinhanh as $ha) {
	...
	}else $d->redirect("index.php?p=phuongtien&a=man&page=".@$_REQUEST['page']);
}
?>