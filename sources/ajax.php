...
if(!isset($_SESSION))
{
	...
	if($do=='recaptcha') {
		echo '<input type="text" class="form-control" required="" id="captcha" name="captcha" style="background: #fff url(./sources/captcha_code.php) center right no-repeat">';
	}
	if($do=='xoa_sp_gh') {
		...
		$id = addslashes($_POST['id']);
		  ...
		  if($sl == 0){
		   unset($_SESSION['cart'][$id]);
		   	...
		   	$_SESSION['cart'][$id] = $sl;
		   	 // $_SESSION['cart'][$id]['soluong'] = $sl; Array price - color
		  } 
		  echo 1;
	}
